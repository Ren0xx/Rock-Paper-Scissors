...
function stopGame() {
  ...
  return computerCounter > playerCounter
    ? (result.textContent = `You've lost this one. Better luck next time!`)
    : (result.textContent = `You've won this one. Congratulations!`);
}
function playGame(howManyGames = 5) {
  buttons.forEach((button) => {
    button.addEventListener("click", () => {
      ...
      if (
        parseInt(playerScore.textContent) === howManyGames ||
        parseInt(computerScore.textContent) === howManyGames
      ) {
        stopGame();
      }
    });
